...
export class RunwareService {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.ws.onmessage = (event) => {
        ...
        if (response.error || response.errors) {
          console.error("Erro na resposta WebSocket:", response);
          const errorMessage = response.errorMessage || response.errors?.[0]?.message || "Ocorreu um erro";
        ...
        if (response.data) {
          response.data.forEach((item: any) => {
            if (item.taskType === "authentication") {
              console.log("Autenticação bem-sucedida, UUID da sessão:", item.connectionSessionUUID);
  ...
  private authenticate(): Promise<void> {
    return new Promise((resolve, reject) => {
      if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
        reject(new Error("WebSocket não está pronto para autenticação"));
        return;
      }
      ...
      console.log("Enviando mensagem de autenticação");
  ...
  async generateImage(params: GenerateImageParams): Promise<GeneratedImage> {
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log("Enviando mensagem de geração de imagem:", message);
